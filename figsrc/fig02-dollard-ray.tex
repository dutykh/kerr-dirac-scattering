...
\begin{scope}[shift={(0,0)}]
  ...
  % axes
  \draw[axis] (0,0) -- (6.6,0) node[slab, anchor=north west, yshift=1mm] {$t$};
  ...
  % stationary ray u = p t, p = kappa/omega < 1 (arrow = future direction)
  \draw[chI, outarr, line width=1.1pt] (0,0) -- (6.15,3.075);
  \node[tlab, chI, anchor=west, rotate=26.6, inner sep=1.5pt] at (1.05,0.68)
    {$u=p_{\omega'}\,t$};
  \node[tlab, chI, anchor=north west] at (1.55,0.42)
    {$p_{\omega'}=\kappa'/\omega'$: massive group velocity};
  % wave packet: support of the spatial-infinity packet at time t, centred on the ray
  \draw[chI, fill=chI!12, rotate around={26.6:(5.0,2.5)}]
    (5.0,2.5) ellipse (0.62 and 0.24);
  \draw[chI, line width=0.4pt] (4.86,2.82) -- (4.55,3.15);
  \node[tlab, chI, anchor=south, align=center] at (4.55,3.20)
    {packet support\\ at time $t$};
